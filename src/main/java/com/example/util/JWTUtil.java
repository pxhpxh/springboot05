...
public class JWTUtil {
    ...
    private static String SECRET="SECRET";//创建加密盐
    ...
    public static String getUserIdFromToken(String token) {
        ...
        try {
            ...
            userId = null;
            log.info("error:{}", "用户未能获取 from token");
        }
        return userId;
    }
    ...
    public static boolean validateToken(String token, String userId) {
        ...
        //我们要做的 是先获取用户名
        String _userId = getUserIdFromToken(token);
        return _userId.equals(userId) && !isTokenExpired(token);
    }
